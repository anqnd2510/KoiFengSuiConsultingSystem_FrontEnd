...
const WorkshopCheck = () => {
  ...
  const fetchPendingWorkshops = async () => {
    try {
      ...
      setError("Không thể tải danh sách hội thảo chờ phê duyệt. Vui lòng thử lại sau.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApproveWorkshop = async (workshopId) => {
    try {
      ...
      if (result) {
        message.success("Đã phê duyệt hội thảo thành công. Trạng thái đã chuyển thành 'Sắp diễn ra'");
      ...
      message.error("Không thể phê duyệt hội thảo: " + (err.message || "Lỗi không xác định"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="p-4">
          <p className="mb-4">Vui lòng nhập lý do từ chối hội thảo "{selectedWorkshop?.name}"</p>
